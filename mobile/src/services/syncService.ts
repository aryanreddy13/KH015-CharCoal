...
class SyncService {
  ...
  init() {
    ...
    this.unsubscribeNetInfo = NetInfo.addEventListener((state: NetInfoState) => {
      ...
      if (connected) {
        console.log('[PS20 SyncService] Network connected. Syncing pending items...');
        this.syncAllPending();
      }
    });
    ...
    try {
      ...
      for (const item of pendingSOS) {
        try {
          ...
          console.warn(`[PS20 SyncService] SOS item ${item.local_id} sync failed:`, err.message);
      ...
      for (const item of pendingReports) {
        try {
          ...
          console.warn(`[PS20 SyncService] Report item ${item.local_id} sync failed:`, err.message);
